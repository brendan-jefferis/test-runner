...
Elm.Component.Counter = Elm.Component.Counter || {};
Elm.Component.Counter.make = function (_elm) {
   "use strict";
   _elm.Component = _elm.Component || {};
   _elm.Component.Counter = _elm.Component.Counter || {};
   if (_elm.Component.Counter.values) return _elm.Component.Counter.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   var update = F2(function (action,model) {
      ...
      {case "NoOp": return model;
         case "Reset": return _U.update(model,{count: 0});
         case "Increment": return _U.update(model,{count: model.count + 1});
         case "Decrement": return _U.update(model,{count: model.count - 1});
         case "Double": return _U.update(model,{count: model.count * 2});
         default: return _U.update(model,{count: $Basics.negate(model.count)});}
   });
   ...
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("counter-app")]),
      _U.list([A2($Html.h2,_U.list([]),_U.list([$Html.text("Counter")]))
              ,A2($Html.p,_U.list([]),_U.list([$Html.text(A2($Basics._op["++"],"Count: ",$Basics.toString(model.count)))]))
              ,A2($Html.button,_U.list([A2($Html$Events.onClick,address,Decrement)]),_U.list([$Html.text("-1")]))
              ,A2($Html.button,_U.list([A2($Html$Events.onClick,address,Increment)]),_U.list([$Html.text("+1")]))
              ,A2($Html.button,_U.list([A2($Html$Events.onClick,address,Double)]),_U.list([$Html.text("x2")]))
              ,A2($Html.button,_U.list([A2($Html$Events.onClick,address,Invert)]),_U.list([$Html.text("Invert")]))
              ,A2($Html.button,_U.list([A2($Html$Events.onClick,address,Reset)]),_U.list([$Html.text("Reset")]))]));
   });
   var NoOp = {ctor: "NoOp"};
   var actions = $Signal.mailbox(NoOp);
   var stringToAction = function (str) {
      var _p1 = str;
      switch (_p1)
   ...
   return _elm.Component.Counter.values = {_op: _op,init: init,update: update,view: view,stringToAction: stringToAction,Model: Model};
};
Elm.Main = Elm.Main || {};
Elm.Main.make = function (_elm) {
   ...
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Component$Counter = Elm.Component.Counter.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   $Html$Attributes = Elm.Html.Attributes.make(_elm),
   $Html$Events = Elm.Html.Events.make(_elm),
   ...
   $String = Elm.String.make(_elm);
   var _op = {};
   var formatListString = function (str) {
      return A4($Regex.replace,$Regex.All,$Regex.regex(","),function (_p0) {    return ", ";},A3($String.slice,1,$String.length(str) - 1,str));
   };
   ...
   var toCounterActions = function (actions) {
      return A2($List.map,$Component$Counter.stringToAction,A2($String.split,",",A3($String.slice,1,$String.length(actions) - 1,actions)));
   };
   var toElmTestCase = function (model) {
      return _U.update(model,
      {name: model.name,initialState: {count: model.initialState.count},finalState: {count: model.finalState.count},actions: toCounterActions(model.actions)});
   };
   var elmTestSuite = function (testCases) {
      return {counter: $Component$Counter.init
             ,actions: _U.list([])
             ,caseName: ""
             ,showTools: false
             ,recording: false
             ,running: false
             ,initialState: $Component$Counter.init
             ,suite: A2($List.map,toElmTestCase,testCases)};
   };
   var sanitizeActions = function (model) {
      return _U.update(model,{name: model.name,initialState: model.initialState,finalState: model.finalState,actions: $Basics.toString(model.actions)});
   };
   var jsTestSuite = function (model) {    return A2($List.map,sanitizeActions,model.suite);};
   var runTest = function (testCase) {
      var result = A3($List.foldl,$Component$Counter.update,testCase.initialState,testCase.actions);
      return _U.update(testCase,{passed: _U.eq(testCase.finalState,result)});
   };
   var newTestCase = function (model) {
      return {name: model.caseName,initialState: model.initialState,finalState: model.counter,actions: model.actions,passed: false};
   };
   var incomingTestSuite = Elm.Native.Port.make(_elm).inbound("incomingTestSuite",
   "Maybe.Maybe\n    (List Main.JSTestCase)",
   function (v) {
      return v === null ? Elm.Maybe.make(_elm).Nothing : Elm.Maybe.make(_elm).Just(typeof v === "object" && v instanceof Array ? Elm.Native.List.make(_elm).fromArray(v.map(function (v) {
         return typeof v === "object" && "name" in v && "initialState" in v && "finalState" in v && "actions" in v && "passed" in v ? {_: {}
                                                                                                                                      ,name: typeof v.name === "string" || typeof v.name === "object" && v.name instanceof String ? v.name : _U.badPort("a string",
                                                                                                                                      v.name)
                                                                                                                                      ,initialState: typeof v.initialState === "object" && "count" in v.initialState ? {_: {}
                                                                                                                                                                                                                       ,count: typeof v.initialState.count === "number" && isFinite(v.initialState.count) && Math.floor(v.initialState.count) === v.initialState.count ? v.initialState.count : _U.badPort("an integer",
                                                                                                                                                                                                                       v.initialState.count)} : _U.badPort("an object with fields `count`",
                                                                                                                                      v.initialState)
                                                                                                                                      ,finalState: typeof v.finalState === "object" && "count" in v.finalState ? {_: {}
                                                                                                                                                                                                                 ,count: typeof v.finalState.count === "number" && isFinite(v.finalState.count) && Math.floor(v.finalState.count) === v.finalState.count ? v.finalState.count : _U.badPort("an integer",
                                                                                                                                                                                                                 v.finalState.count)} : _U.badPort("an object with fields `count`",
                                                                                                                                      v.finalState)
                                                                                                                                      ,actions: typeof v.actions === "string" || typeof v.actions === "object" && v.actions instanceof String ? v.actions : _U.badPort("a string",
                                                                                                                                      v.actions)
                                                                                                                                      ,passed: typeof v.passed === "boolean" ? v.passed : _U.badPort("a boolean (true or false)",
                                                                                                                                      v.passed)} : _U.badPort("an object with fields `name`, `initialState`, `finalState`, `actions`, `passed`",
         v);
      })) : _U.badPort("an array",v));
   });
   var resultView = function (model) {
      return A2($Html.tr,
      _U.list([$Html$Attributes.$class(model.passed ? "test pass" : "test fail")]),
      _U.list([A2($Html.td,_U.list([$Html$Attributes.$class("name")]),_U.list([$Html.text(model.name)]))
              ,A2($Html.td,_U.list([$Html$Attributes.$class("result")]),_U.list([$Html.text(model.passed ? "Passed" : "Fail")]))]));
   };
   var testCaseView = function (model) {
      return A2($Html.tr,
      _U.list([$Html$Attributes.$class("test-case")]),
      _U.list([A2($Html.td,_U.list([$Html$Attributes.$class("name")]),_U.list([$Html.text(model.name)]))
              ,A2($Html.td,_U.list([$Html$Attributes.$class("init-state")]),_U.list([$Html.text($Basics.toString(model.initialState))]))
              ,A2($Html.td,_U.list([$Html$Attributes.$class("actions")]),_U.list([$Html.text(formatListString($Basics.toString(model.actions)))]))
              ,A2($Html.td,_U.list([$Html$Attributes.$class("final-state")]),_U.list([$Html.text($Basics.toString(model.finalState))]))]));
   };
   var update = F2(function (action,model) {
      var _p1 = action;
      switch (_p1.ctor)
      {case "NoOp": return model;
         case "Counter": var _p2 = _p1._0;
           var actions = model.recording ? A2($Basics._op["++"],model.actions,_U.list([_p2])) : model.actions;
           return _U.update(model,{counter: A2($Component$Counter.update,_p2,model.counter),actions: actions});
         case "SetCaseName": return _U.update(model,{caseName: _p1._0});
         case "ToggleTestTools": return _U.update(model,{showTools: $Basics.not(model.showTools)});
         case "Record": return _U.update(model,{recording: true,showTools: false,caseName: model.caseName,initialState: model.counter});
         case "Cancel": return _U.update(model,{recording: false,caseName: "",actions: _U.list([]),initialState: model.counter});
         case "Save": var suite = A2($Basics._op["++"],model.suite,_U.list([newTestCase(model)]));
           return _U.update(model,{recording: false,showTools: true,caseName: "",actions: _U.list([]),initialState: $Component$Counter.init,suite: suite});
         case "DeleteAll": return _U.update(model,{suite: _U.list([])});
         default: var suite = A2($List.map,runTest,model.suite);
           return _U.update(model,{running: true,suite: suite});}
   });
   var Run = {ctor: "Run"};
   var DeleteAll = {ctor: "DeleteAll"};
   var Cancel = {ctor: "Cancel"};
   var Save = {ctor: "Save"};
   var Record = {ctor: "Record"};
   var ToggleTestTools = {ctor: "ToggleTestTools"};
   var testToolBarView = F2(function (address,model) {
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("tool-bar")]),
      _U.list([A2($Html.div,_U.list([$Html$Attributes.$class(A2($Basics._op["++"],"icon-record",model.recording ? " recording" : ""))]),_U.list([]))
              ,A2($Html.span,_U.list([$Html$Attributes.$class("recording-status")]),_U.list([$Html.text(model.recording ? "Recording..." : "Not recording")]))
              ,A2($Html.button,
              _U.list([A2($Html$Events.onClick,address,ToggleTestTools),$Html$Attributes.$class("button-tiny show-tools")]),
              _U.list([$Html.text(model.showTools ? "Hide tools" : "Show tools")]))
              ,A2($Html.button,
              _U.list([A2($Html$Events.onClick,address,Save),$Html$Attributes.$class("button-tiny"),$Html$Attributes.hidden($Basics.not(model.recording))]),
              _U.list([$Html.text("Save test")]))
              ,A2($Html.button,
              _U.list([A2($Html$Events.onClick,address,Cancel),$Html$Attributes.$class("button-tiny"),$Html$Attributes.hidden($Basics.not(model.recording))]),
              _U.list([$Html.text("Cancel")]))]));
   });
   var SetCaseName = function (a) {    return {ctor: "SetCaseName",_0: a};};
   var testToolsView = F2(function (address,model) {
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("test-tools")]),
      _U.list([A2($Html.label,_U.list([]),_U.list([$Html.text("Test name")]))
              ,A2($Html.input,
              _U.list([A2(onInput,address,SetCaseName)
                      ,$Html$Attributes.value(model.caseName)
                      ,$Html$Attributes.type$("text")
                      ,$Html$Attributes.$class("case-name")
                      ,$Html$Attributes.placeholder("e.g. When user clicks x result should be y")]),
              _U.list([]))
              ,A2($Html.button,
              _U.list([A2($Html$Events.onClick,address,Record),$Html$Attributes.disabled(_U.eq(model.caseName,""))]),
              _U.list([$Html.text(model.recording ? "Save" : "Record")]))
              ,A2($Html.button,
              _U.list([A2($Html$Events.onClick,address,Run),$Html$Attributes.disabled(_U.eq($List.length(model.suite),0))]),
              _U.list([$Html.text("Run tests")]))
              ,A2($Html.button,
              _U.list([A2($Html$Events.onClick,address,DeleteAll),$Html$Attributes.disabled(_U.eq($List.length(model.suite),0))]),
              _U.list([$Html.text("Delete tests")]))
              ,$List.isEmpty(model.suite) ? A2($Html.div,_U.list([]),_U.list([])) : A2($Html.table,
              _U.list([]),
              _U.list([A2($Html.thead,
                      _U.list([]),
                      _U.list([A2($Html.th,_U.list([]),_U.list([$Html.text("Name")]))
                              ,A2($Html.th,_U.list([]),_U.list([$Html.text("Given")]))
                              ,A2($Html.th,_U.list([]),_U.list([$Html.text("When")]))
                              ,A2($Html.th,_U.list([]),_U.list([$Html.text("Then")]))]))
                      ,A2($Html.tbody,_U.list([]),A2($List.map,testCaseView,model.suite))]))
              ,model.running && $Basics.not($List.isEmpty(model.suite)) ? A2($Html.table,
              _U.list([$Html$Attributes.$class("results")]),
              _U.list([A2($Html.thead,
                      _U.list([]),
                      _U.list([A2($Html.th,_U.list([]),_U.list([$Html.text("Case name")])),A2($Html.th,_U.list([]),_U.list([$Html.text("Result")]))]))
                      ,A2($Html.tbody,_U.list([]),A2($List.map,resultView,model.suite))])) : A2($Html.div,_U.list([]),_U.list([]))]));
   });
   ...
   var view = F2(function (address,model) {
      ...
      _U.list([]),
      _U.list([A2($Html.div,
              _U.list([$Html$Attributes.$class("test-runner")]),
              _U.list([A2(testToolBarView,address,model),model.showTools ? A2(testToolsView,address,model) : A2($Html.div,_U.list([]),_U.list([]))]))
              ,A2($Component$Counter.view,A2($Signal.forwardTo,address,Counter),model.counter)]));
   });
   var NoOp = {ctor: "NoOp"};
   var actions = $Signal.mailbox(NoOp);
   var JSTestCase = F5(function (a,b,c,d,e) {    return {name: a,initialState: b,finalState: c,actions: d,passed: e};});
   var TestCase = F5(function (a,b,c,d,e) {    return {name: a,initialState: b,finalState: c,actions: d,passed: e};});
   var emptyModel = {counter: $Component$Counter.init
                    ,actions: _U.list([])
                    ,caseName: ""
                    ,showTools: false
                    ,recording: false
                    ...
                    ,initialState: $Component$Counter.init
                    ,suite: _U.list([])};
   var initialModel = A2($Maybe.withDefault,emptyModel,A2($Maybe.map,elmTestSuite,incomingTestSuite));
   var model = A3($Signal.foldp,update,initialModel,actions.signal);
   var main = A2($Signal.map,view(actions.address),model);
   var outgoingTestSuite = Elm.Native.Port.make(_elm).outboundSignal("outgoingTestSuite",
   function (v) {
      return Elm.Native.List.make(_elm).toArray(v).map(function (v) {
         return {name: v.name,initialState: {count: v.initialState.count},finalState: {count: v.finalState.count},actions: v.actions,passed: v.passed};
      });
   },
   A2($Signal.map,jsTestSuite,model));
   var Model = F8(function (a,b,c,d,e,f,g,h) {    return {counter: a,actions: b,caseName: c,showTools: d,recording: e,running: f,initialState: g,suite: h};});
   return _elm.Main.values = {_op: _op
                             ...
                             ,ToggleTestTools: ToggleTestTools
                             ,Record: Record
                             ,Save: Save
                             ,Cancel: Cancel
                             ,DeleteAll: DeleteAll
                             ...
                             ,update: update
                             ,view: view
                             ,testToolBarView: testToolBarView
                             ,testToolsView: testToolsView
                             ...
                             ,toElmTestCase: toElmTestCase
                             ,jsTestSuite: jsTestSuite
                             ,elmTestSuite: elmTestSuite
                             ,toCounterActions: toCounterActions
                             ,onInput: onInput
                             ,formatListString: formatListString};
};
